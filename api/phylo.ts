// This is a Vercel Serverless Function to run Phylogenetic Tree generation via EMBL-EBI API.
...
export default async function handler(req: any, res: any) {
    ...
    try {
        if (jobId) {
            ...
            if (status === 'FINISHED') {
                const resultResponse = await fetch(`https://www.ebi.ac.uk/Tools/services/rest/clustalo/result/${jobId}/phylotree`);
                if (!resultResponse.ok) {
                    throw new Error(`Failed to fetch results: ${await resultResponse.text()}`);
                }
                const treeText = await resultResponse.text();
                return res.status(200).json({ status: 'FINISHED', result: treeText });
